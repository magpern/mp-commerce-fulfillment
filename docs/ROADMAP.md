...
6. **M6 — Package photography** — **M6-A foundation + M6-B REST/guard**
   on `feature/m6-package-photography` (not merged). Persistence, domain,
   protected storage, GD processing v1, `PhotoService`, audit events,
   photo REST + streaming, `photos_required` packing guard wiring.
   Workspace UI / settings UI / retention purge / `v0.6.0` remain M6-C–D.
   Evidence: `docs/ARCHITECTURE_PLAN.md` Part VIII.10–VIII.11.
