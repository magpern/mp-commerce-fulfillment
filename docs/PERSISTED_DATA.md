...
| `mpcf_media` | 6 | Package photography evidence (Part VIII) — one row per capture. `package_id` NOT NULL; relative `file_path` / `thumb_path` under `uploads/mpcf/photos/…` (cleared to empty string after retention purge); integrity columns `mime`, `bytes`, `sha256`, `processing_version`, dimensions retained forever; soft-delete via `deleted_at` (bytes preserved until purge); retention marker `purged_at` after byte removal. Indexes: `fulfillment_id`, `package_id`, `(fulfillment_id, deleted_at)`, `(package_id, deleted_at)`, `(fulfillment_id, seq)`. |
| `mpcf_waves` | 7 | Wave aggregate (Part X) — warehouse walk over many fulfillments. Columns: `warehouse_id`, `owner_user_id`, `state`, optimistic `version`, `title`, `settings_snapshot`, lifecycle timestamps. Indexes: `(state, warehouse_id, owner_user_id)`, `updated_at`. |
| `mpcf_wave_members` | 7 | Wave membership — `(wave_id, fulfillment_id)` PK; `position`, `joined_at`, `picked_at`. Index: `fulfillment_id`. Open-wave exclusivity enforced in Application. |
| `mpcf_analytics_daily` | 8 | UTC-day × warehouse analytics rollups (Part XI). Columns: `utc_date`, `warehouse_id`, `rollup_version`, `counters_json`, `durations_json`, `source_event_max_id`, `computed_at`, `created_at`, `updated_at`. Unique: `(utc_date, warehouse_id)`. No PII. |
...
|---|---|---|
| `mpcf` | `mpcf_process_intake` (the Action Scheduler fallback path `Woo\IntakeHooks` uses when synchronous intake can't complete inline); `mpcf_purge_photo_retention` (M6 photo retention); `mpcf_analytics_daily_rollup` (M9 nightly ROLLUP of closed UTC days) | `Woo\IntakeHooks`, `PhotoRetentionScheduler`, `AnalyticsRollupScheduler` |
...
Protected upload root under `wp-content/uploads/mpcf/` (ADR-0004). M4
