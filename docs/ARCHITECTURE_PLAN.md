...
Both steps are additive-only (`ALTER TABLE ... ADD KEY`/`ADD UNIQUE KEY`), idempotent (guarded by `SHOW INDEX` checks), and covered by migration tests. D21's 10k-row proof (III.5 criterion 3) still runs against the real Queue query shapes as originally planned — this reorders when two of the needed indexes were discovered, it does not skip the proof itself.
