...
final class WpdbAnalyticsDailyRepository implements AnalyticsDailyRepository {
	...
	public function find( string $utc_date, int $warehouse_id ): ?DailyMetrics {
		...
		$row = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$table} WHERE `utc_date` = %s AND warehouse_id = %d LIMIT 1", $utc_date, $warehouse_id ), ARRAY_A );
	...
	public function find_range( string $from_utc_date, string $to_utc_date_inclusive, int $warehouse_id ): array {
		...
		$table = Schema::table( Schema::ANALYTICS_DAILY );
		// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Schema-built table name.
		$rows = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$table} WHERE warehouse_id = %d AND `utc_date` >= %s AND `utc_date` <= %s ORDER BY `utc_date` ASC", $warehouse_id, $from_utc_date, $to_utc_date_inclusive ), ARRAY_A );
	...
	public function upsert( DailyMetrics $metrics, string $computed_at_utc ): void {
		...
		if ( null === $existing ) {
			// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Schema-built table name.
			$wpdb->query( $wpdb->prepare( "INSERT INTO {$table} (`utc_date`, warehouse_id, rollup_version, counters_json, durations_json, source_event_max_id, computed_at, created_at, updated_at) VALUES (%s, %d, %d, %s, %s, %s, %s, %s, %s)", $metrics->utc_date(), $metrics->warehouse_id(), $metrics->rollup_version(), wp_json_encode( $metrics->counters() ), wp_json_encode( $metrics->durations() ), null === $max_id ? null : (string) $max_id, $now, $now, $now ) );
			return;
		}

		// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Schema-built table name.
		$wpdb->query( $wpdb->prepare( "UPDATE {$table} SET rollup_version = %d, counters_json = %s, durations_json = %s, source_event_max_id = %s, computed_at = %s, updated_at = %s WHERE `utc_date` = %s AND warehouse_id = %d", $metrics->rollup_version(), wp_json_encode( $metrics->counters() ), wp_json_encode( $metrics->durations() ), null === $max_id ? null : (string) $max_id, $now, $now, $metrics->utc_date(), $metrics->warehouse_id() ) );
	}
